...
D3DX10INLINE void OutputVector(D3DXVECTOR3 x) { if(x){ OutputDebugString( (std::to_wstring(x.x) + L", " + std::to_wstring(x.y) + L", " + std::to_wstring(x.z) + L"\n").c_str()) ;} }
...
template<typename T>
D3DX10INLINE T Restrict(const T& x, const T& low, const T& high)
{
	return x < low ? high : (x > high ? low : x);
}

D3DX10INLINE float VectorLength(const D3DXVECTOR3& x)
{
	return abs(sqrt(pow(x.x, 2) + pow(x.y, 2) + pow(x.z, 2)));
}
...
struct CLine {
	CLine() {};
	CLine(D3DXVECTOR3 _pos1, D3DXVECTOR3 _pos2) :pos1(_pos1), pos2(_pos2) {};

	D3DXVECTOR3 pos1;
	D3DXVECTOR3 pos2;
};

enum class ERigidity {
	R_DEFAULT = 0,
	R_CROSSED,
	R_INTERWEAVED
};

enum class ECollidible {
	CL_NONE = 0,
	CL_SPHERE,
	CL_CAPSULE,
	CL_PYRAMID
